...
Video files carry a map. An MP4 -- what our masters are -- contains an index, the moov atom, recording where every moment of the video lives in the file, down to the byte.<Note id="2" /> Read the index and you know which slice of a 4 GB file holds minute 133.
...
Near-black detection uses YHIGH -- the 90th-percentile luma -- to catch dead-signal black frames.<Note id="5" /> The threshold is 32 (TV black 16 plus analog noise headroom). Real content runs YHIGH 110-214, so the filter drops only dead frames.
...
<Figure
  src="/images/writing/one-frame-from-a-four-gigabyte-video/rejected-frames.png"
  alt="2x3 grid of synthetic test frames: solid blue, solid black, near-black, solid green, and two SMPTE color-bar frames"
  caption="Synthetic test fixtures through the at=auto filter: blue tape leader, dead-signal black, near-black fade, a stand-in good frame, and SMPTE bars. The filter rejects the blue and black frames; the bars and the good frame survive."
/>
